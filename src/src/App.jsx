...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'Jost', sans-serif", background: "var(--cream)", minHeight: "100vh", color: "var(--espresso)" }}>
      ...
      {orderStatus === "success" && (
        <div style={{ background: "var(--espresso)", color: "var(--cream-card)", textAlign: "center", padding: "14px 20px", fontSize: 14 }}>
          Order received — thank you! You'll be contacted to confirm the details.
          <button className="btn" style={{ background: "transparent", color: "var(--cream-card)", textDecoration: "underline", marginLeft: 10, padding: 0, fontSize: 13 }} onClick={() => setOrderStatus(null)}>Dismiss</button>
        </div>
      )}
      {orderStatus === "cancelled" && (
        <div style={{ background: "var(--rose)", color: "var(--cream-card)", textAlign: "center", padding: "14px 20px", fontSize: 14 }}>
          Checkout was cancelled — no payment was made. Your cart items were cleared, so feel free to add them again.
          <button className="btn" style={{ background: "transparent", color: "var(--cream-card)", textDecoration: "underline", marginLeft: 10, padding: 0, fontSize: 13 }} onClick={() => setOrderStatus(null)}>Dismiss</button>
        </div>
      )}
